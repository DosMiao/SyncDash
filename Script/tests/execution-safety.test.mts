...
test('the reviewed-set identity changes for every decision that can change writes', () => {
  const base = reviewedSetKey(owner, 'photos', 'rev-2', 1, [{ index: 2, flipped: false }]);

  assert.notEqual(base, reviewedSetKey({ ...owner, compare_id: 18 }, 'photos', 'rev-2', 1, [{ index: 2, flipped: false }]));
  assert.notEqual(base, reviewedSetKey(owner, 'photos', 'rev-3', 1, [{ index: 2, flipped: false }]));
  assert.notEqual(base, reviewedSetKey(owner, 'photos', 'rev-2', 0, [{ index: 2, flipped: false }]));
  assert.notEqual(base, reviewedSetKey(owner, 'photos', 'rev-2', 1, [{ index: 3, flipped: false }]));
  assert.notEqual(base, reviewedSetKey(owner, 'photos', 'rev-2', 1, [{ index: 2, flipped: true }]));
  assert.equal(base, reviewedSetKey(owner, 'photos', 'rev-2', 1, [{ index: 2, flipped: false }]));
});
...
test('auto-apply accepts only the result owned by the currently armed cycle and selection', () => {
  ...
  const selection = { jobName: 'photos', configRevision: 'rev-2', targetIndex: 1 };
