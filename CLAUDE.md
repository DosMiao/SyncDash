...
- Build: `npm run build && cargo build --release -p syncdash-desktop` (desktop) · `cargo build --release -p syncdash` (CLI). Windows `builder.bat` and macOS `builder.command` are thin launchers for the one Rust adapter under `tools/builder`, using the shared core in the sibling `Experience/builder` repository. The Mac menu's `[2]`/`[3]`/`[4]` remain pure cargo over the committed `dist/` and need no node; `[1]` Dev and `[A]` App do, because tauri runs `beforeBuildCommand` on both.
