...
- User instructions may be in Chinese. Always respond in English. **Everything human-readable in the product is English too** — code, identifiers, comments, documentation, persistent memory entries, `--help` text and user-facing UI strings — spelling **en-US**.
...
- **No defensive fallbacks, no partial states.** This is the product's founding principle, not a style preference: a sync tool that guesses destroys data. Never substitute a default for a missing or failed value (`?? d`, `.unwrap_or(default)`, empty `catch`) — fail loudly at the broken site. The reference behavior is `vfs::spec`: an unknown `xyz://` scheme parses to `UnknownScheme` rather than falling through to "local path", because treating `sfpt://host/x` as relative would create a directory literally named `sfpt:`. Derived surfaces gate on the COMPLETE input set and compute once — show "waiting", never partial or progressive rendering.
...
- **`core/runScope.ts` owns execution membership; `core/grouping.ts` owns display order.** `computeInScopeIndices` returns engine plan order, while grouping/sorting produce a separate display order for the table and CSV.
- `core/folders.ts` is the neutral shared folder contract: root sentinel, root label and operation ownership. Run Scope and table grouping may keep different aggregate models, but neither may reimplement those three semantics.
- `computeExecutableIndices` filters the in-scope plan-order indices, never `layout.displayOrder` — directory deletion must follow its children. Differences / Identical, sorting, folding, grouping, and path display are presentation only.
- `useRunScopeController` is the single owner of result-type/folder/advanced criteria, draft/applied search, mask request fencing, and the one-time `sd.ov` → `sd.scope` preference migration. Mask matching is fail-closed: pending or failed masks produce no executable rows.
- Manual Apply is available only in the Differences result view. `deriveApplyAvailability` drives the toolbar and the confirmation guard, so a hidden Differences action set cannot run while Identical is open.
- No ESLint config exists yet, so `react-hooks/exhaustive-deps` has never run — check hook dependencies by hand.
- `App.tsx` remains the large orchestration shell (~2,260 lines / 89 hook calls). Result-domain work belongs in framework-free `core/`, state machines in `ui/hooks` or `ui/state`, and rendering in named components; do not bridge new behavior back into the shell with serialized reset keys or duplicated state.
...
- `typescript/styles.css` is the single token layer; its header states the four rules and they are binding. **No literal color outside the token block, and no literal px font-size outside the `--fs-*` scale** — a rule needing a value the scale lacks means the scale is wrong, not the rule.
...
- Palette and page structure follow GitHub Primer; component vocabulary (`.btn` / `.chip` / `.badge` / `.menu-*` / `.sheet`) is GitDash's. Light-theme hues take a darker step than Primer publishes, because Primer's own success/attention/done measure 4.29–4.48:1 on the hover surface. **A reference palette is a starting point, not a pass** — after any color change re-run the contrast audit against `--bg`, `--bg-2` **and** `--bg-3`; several hues clear the first and fail the third.
- Measure before acting on an impression. "Looks hazy" turned out to mean the page was twice as bright (canvas luminance 0.0117 → 0.0055), not that the grays were too close; acting on the intuition would have made it worse.
- Result types are defined once by `RESULT_TYPES` + exhaustive `RESULT_TYPE_DEFINITIONS`; React adds the exhaustive `RESULT_TYPE_ICON` map. Rows and Run Scope share all six types, while toolbar stats intentionally use only the executable four. `.result-type-*` classes route one `--result-type-color` token to every surface.
