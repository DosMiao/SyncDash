...
pub const BASE: &[Seed] = &[
    Seed { path: "docs/readme.md", seed: 1, size: 1_024, mtime_ms: T0 },
    Seed { path: "docs/manual.md", seed: 2, size: 4_096, mtime_ms: T0 + 1_000 },
    Seed { path: "media/thumb.png", seed: 3, size: 65_536, mtime_ms: T0 + 2_000 },
    Seed { path: "code/lib/core.rs", seed: 4, size: 8_192, mtime_ms: T0 + 3_000 },
    Seed { path: "code/lib/util.rs", seed: 5, size: 8_192, mtime_ms: T0 + 4_000 },
    Seed { path: "nested/a/b/c/deep.txt", seed: 6, size: 1_024, mtime_ms: T0 + 5_000 },
    Seed { path: "archive-me/old1.txt", seed: 7, size: 2_048, mtime_ms: T0 + 6_000 },
    Seed { path: "archive-me/old2.txt", seed: 8, size: 2_048, mtime_ms: T0 + 7_000 },
    ...
    Seed { path: "empty/zero-1.dat", seed: 0, size: 0, mtime_ms: T0 + 8_000 },
    // Three byte-identical files: the only way to give `detect_moves` a bucket big enough to
    // report an ambiguous pairing.
    Seed { path: "dupes/twin-1.bin", seed: 99, size: 4_096, mtime_ms: T0 + 9_000 },
    Seed { path: "dupes/twin-2.bin", seed: 99, size: 4_096, mtime_ms: T0 + 10_000 },
    Seed { path: "dupes/twin-3.bin", seed: 99, size: 4_096, mtime_ms: T0 + 11_000 },
];
...
pub const SAMPLING: &[Seed] = &[
    Seed { path: "big/handbook.bin", seed: 20, size: 6 * 1_048_576, mtime_ms: T0 },
    Seed { path: "big/at-4mib.bin", seed: 21, size: 4 * 1_048_576, mtime_ms: T0 + 1_000 },
    Seed { path: "big/at-4mib-minus1.bin", seed: 22, size: 4 * 1_048_576 - 1, mtime_ms: T0 + 2_000 },
];
...
fn write(v: &Arc<dyn Vfs>, rel: &str, content: &[u8], mtime_ms: i64) {
    if let Some(parent) = crate::foundation::path::parent(rel) {
        v.mkdir_all(parent).unwrap_or_else(|e| panic!("mkdir_all({parent:?}): {e}"));
    }
    let hint = WriteHint { size_hint: Some(content.len() as u64), mtime_ms: Some(mtime_ms), mode: None };
    let mut w = v.open_write(rel, &hint).unwrap_or_else(|e| panic!("open_write({rel:?}): {e}"));
    w.write(content).unwrap_or_else(|e| panic!("write({rel:?}): {e}"));
    w.seal(false).unwrap_or_else(|e| panic!("seal({rel:?}): {e}"));
    w.commit().unwrap_or_else(|e| panic!("commit({rel:?}): {e}"));
    ...
    if v.caps().set_mtime.yes() {
        v.set_mtime(rel, mtime_ms).unwrap_or_else(|e| panic!("set_mtime({rel:?}): {e}"));
    }
}
...
pub fn apply_edits(v: &Arc<dyn Vfs>, edits: &[Edit]) {
    for e in edits {
        match *e {
            Edit::Add(s) | Edit::Rewrite(s) => write(v, s.path, &filler(s.seed, s.size), s.mtime_ms),
            Edit::Patch { path, at, xor } => {
                let mut data = read(v, path);
                assert!(at < data.len(), "patch offset {at} is past the end of {path} ({})", data.len());
            ...
            Edit::Rename { from, to } => {
                ...
                let mtime = v.stat(from).unwrap().expect("rename source exists").mtime_ms;
                write(v, to, &data, mtime);
                v.remove_file(from).unwrap_or_else(|e| panic!("remove_file({from:?}): {e}"));
            }
            Edit::Delete(path) => {
                v.remove_file(path).unwrap_or_else(|e| panic!("remove_file({path:?}): {e}"));
            }
            Edit::Touch { path, mtime_ms } => {
                v.set_mtime(path, mtime_ms).unwrap_or_else(|e| panic!("set_mtime({path:?}): {e}"));
            }
            Edit::Chmod { path, mode } => {
                v.set_mode(path, mode).unwrap_or_else(|e| panic!("set_mode({path:?}): {e}"));
            }
