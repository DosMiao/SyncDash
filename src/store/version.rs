...
impl VersionWriter {
    ...
    pub fn preserve(
        ...
        let try_rdelta = !is_link
            && old_size >= crate::model::chunk::DELTA_MIN_SIZE
            && new_content.map(|p| p.is_file()).unwrap_or(false);
        ...
        if try_rdelta {
            ...
            let mut new_by_hash: std::collections::HashMap<&str, (u64, u32)> =
            ...
            for c in crate::model::chunk::chunk_bytes(&old_data) {
                if let Some(&(noff, nlen)) = new_by_hash.get(c.hash.as_str()) {
                    ...
                    blob.extend_from_slice(
                        &old_data[c.off as usize..(c.off + c.len as u64) as usize],
                    );
                    recipe.push(RecipeStep {
                        s: "blob".into(),
                        off,
        ...
        let res: std::io::Result<()> = (|| {
            ...
            if std::fs::symlink_metadata(&dst).is_ok() {
                ...
                if e.kind == "rdelta" {
                    ...
                    if Some(cur_hash.as_str()) != e.new_hash.as_deref() {
                        return Err(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "current file no longer matches recorded new_hash",
                        ));
                    }
                    std::fs::copy(&dst, &tp)?;
                } else {
                    std::fs::rename(&dst, &tp).or_else(|_| {
                        std::fs::copy(&dst, &tp)
                            .map(|_| ())
                ...
                return Err(std::io::Error::new(
                    ...
                    "rdelta base (current file) is gone",
                ));
            }
            if e.kind == "rdelta" {
                ...
                if let Some(recipe) = &e.recipe {
                    for st in recipe {
                        ...
                        let slice = if src == "base" {
                            base.get(off..off + len)
                        } else {
                ...
                if got != e.old_hash {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "reconstructed old content hash mismatch",
                    ));
                }
            ...
            let ft = filetime::FileTime::from_unix_time(
                e.old_mtime_ms / 1000,
                ((e.old_mtime_ms % 1000) * 1_000_000) as u32,
            );
