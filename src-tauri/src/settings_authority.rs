//! One-use authority for changing the application log directory.
...
impl SettingsAuthority {
    pub(crate) fn issue_log_directory_grant(
        ...
        let grant_id =
            crate::authority_token::random_hex::<16>("Cannot create a directory-selection grant")?;
        let now = Instant::now();
        let mut grants = self
            .grants
            .lock()
    ...
    pub(crate) fn consume_log_directory_grant(
        ...
        if grant.settings_revision != expected_settings_revision {
            return Err(
                "Settings changed after the log directory was selected — reload settings and choose it again"
                    .into(),
            );
        }
        if grant.directory != directory {
            return Err(
                "The requested log directory does not match the directory selected in the native picker"
                    .into(),
            );
        }
        Ok(())
