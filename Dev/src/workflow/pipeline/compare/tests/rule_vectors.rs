...
const HEADER: &str = concat!(
    ...
    "// Dev/typescript/core/domain/compare/plan.ts re-implements for the desktop table, plus the\n",
    "// compare-policy constant the job editor spells out to the operator.\n",
...
/// The floor is emitted rather than re-typed because the window's whole hazard is a second copy
/// that quietly disagrees. Nothing in the frontend may judge one comparison's rows by it: a run
/// widens the window to the coarser backend's precision and publishes the result as
/// `PlanDto.mtime_window_ms`.
const FLOOR_DOC: &str = concat!(
    "\n",
    "/// The mtime equality window's policy floor, in milliseconds — FAT/SMB timestamp granularity.\n",
    "/// It describes the product's default, not any one comparison: a run widens the window to the\n",
    "/// coarser of the two backends' declared mtime precision and publishes what it used as\n",
    "/// `PlanDto.mtime_window_ms`. Judge a result's rows by that value; use this one only to state\n",
    "/// the configured floor.\n",
    "export const MTIME_WINDOW_FLOOR_MS = ",
);
...
fn export_bindings_compare_plan_rule_vectors() {
    ...
    body.push_str("];\n");
    body.push_str(FLOOR_DOC);
    body.push_str(&format!("{};\n", crate::pipeline::compare::MTIME_SLACK_MS));
