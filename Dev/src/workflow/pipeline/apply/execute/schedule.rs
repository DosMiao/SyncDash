...
use super::super::lease::ApplyLeaseGuard;
use super::super::ApplyOptions;
use super::exec_op;
...
pub(in crate::pipeline::apply) struct Shared<'a> {
    pub(in crate::pipeline::apply) opt: &'a ApplyOptions,
    pub(in crate::pipeline::apply) ctx: &'a RunCtx,
    pub(in crate::pipeline::apply) lease: &'a ApplyLeaseGuard,
    pub(in crate::pipeline::apply) source: &'a std::sync::Arc<dyn crate::fs::vfs::Vfs>,
    pub(in crate::pipeline::apply) target: &'a std::sync::Arc<dyn crate::fs::vfs::Vfs>,
    pub(in crate::pipeline::apply) source_local_root: Option<crate::fs::local_root::LocalRoot>,
    pub(in crate::pipeline::apply) target_local_root: Option<crate::fs::local_root::LocalRoot>,
    ...
    pub(in crate::pipeline::apply) source_trash_ok: bool,
    pub(in crate::pipeline::apply) target_trash_ok: bool,
    pub(in crate::pipeline::apply) central_trash_root: Option<crate::fs::local_root::LocalRoot>,
    ...
    pub(in crate::pipeline::apply) in_root_keep_rel: String,
    pub(in crate::pipeline::apply) central_preserved: AtomicBool,
    pub(in crate::pipeline::apply) source_in_root_preserved: AtomicBool,
    pub(in crate::pipeline::apply) target_in_root_preserved: AtomicBool,
    pub(in crate::pipeline::apply) ver_source: Mutex<Option<crate::store::version::VersionWriter>>,
    pub(in crate::pipeline::apply) ver_target: Mutex<Option<crate::store::version::VersionWriter>>,
    /// Directories already ensured on each side this run (spares one round-trip per network root)
    pub(in crate::pipeline::apply) mkdir_memo: Mutex<std::collections::HashSet<(bool, String)>>,
    // Preserve rounded on-disk and intended mtimes so the next scan can reconcile them exactly.
    pub(in crate::pipeline::apply) mtime_fixes: Mutex<Vec<(bool, String, i64, i64)>>,
    pub(in crate::pipeline::apply) delta_saved: AtomicU64,
}

impl<'a> Shared<'a> {
    pub(in crate::pipeline::apply) fn checkpoint(
        &self,
        pp: &PhaseProgress<'_>,
    ) -> std::io::Result<()> {
    ...
    pub(in crate::pipeline::apply) fn check_before_mutation(&self) -> std::io::Result<()> {
        self.lease.check_before_mutation()
    }

    pub(in crate::pipeline::apply) fn lease_lost(&self) -> bool {
        self.lease.lost()
    }

    pub(in crate::pipeline::apply) fn exec_other(
        &self,
        side: &Side,
    ) -> (
    ...
    pub(in crate::pipeline::apply) fn local_root_of(
        &self,
        side: &Side,
    ) -> Option<&crate::fs::local_root::LocalRoot> {
        match side {
    ...
    pub(in crate::pipeline::apply) fn trash_reaches(&self, side: &Side) -> bool {
        match side {
    ...
    pub(in crate::pipeline::apply) fn note_central_preservation(&self) {
        self.central_preserved.store(true, Ordering::Relaxed);
    }

    pub(in crate::pipeline::apply) fn note_in_root_preservation(&self, side: &Side) {
        match side {
    ...
    pub(in crate::pipeline::apply) fn central_preservation_used(&self) -> bool {
        self.central_preserved.load(Ordering::Relaxed)
    }

    pub(in crate::pipeline::apply) fn in_root_preservation_used(&self, side: &Side) -> bool {
        match side {
    ...
    pub(in crate::pipeline::apply) fn ensure_dir(
        &self,
        side: &Side,
...
#[derive(Default)]
pub(in crate::pipeline::apply) struct Counters {
    pub(in crate::pipeline::apply) done: AtomicU64,
    pub(in crate::pipeline::apply) skipped: AtomicU64,
    pub(in crate::pipeline::apply) errors: AtomicU64,
    pub(in crate::pipeline::apply) lease_failure_recorded: AtomicBool,
}
...
pub(in crate::pipeline::apply) fn run_class(
    class: &[&Op],
    width: usize,
    sh: &Shared,
    pp: &PhaseProgress,
